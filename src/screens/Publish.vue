...
async function init() {
    ...
    store.loadProjectCover(project.value)
}
...
<template>
    <div v-if="project" class="h-[80vh] flex flex-col">
        <div class="flex mt-[50px] border-t border-white ">
            <img @click="store.setProjectCover(project)" :src="project.coverUrl || DefaultCover"
                class="w-[80px] object-cover" />
            <div class="relative bg-gray-300 h-[80px] flex-1">
